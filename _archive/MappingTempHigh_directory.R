...
# Register Google Maps API key you will need to set your own 
...
# Promps you to select the shapefile
shapefile_path <- choose.files(caption = "Select Shapefile")

# Define temperature bins and labels
bins <- c(-Inf, seq(27, 36, by = 1), Inf)
labels <- c(paste("<", 27, sep = ""), paste(seq(27, 35, by = 1), seq(28, 36, by = 1), sep = "-"), paste(">", 36, sep = ""))
...
process_csv <- function(file_path, shapefile_path, output_directory) {
  ...
  plot <- ggmap(google_map) +
    ...
    scale_fill_manual(values = varied_color_palette, name = "Max Temperature (°C)", drop = FALSE) +  # Use fixed colors
    guides(
      fill = guide_legend(override.aes = list(shape = 21, size = 6)),
      shape = guide_legend(override.aes = list(size = 6))
    ) +
    labs(title = paste("Max Temperature Map", date_str),
